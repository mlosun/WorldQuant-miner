...
class ContinuousAlphaExpressionMiner:
    ...
    def mine_alpha_expression(self, expression):
        ...
        try:
            ...
            if removed:
                logger.info(
                    f"Successfully removed alpha '{expression}' from hopeful_alphas.json"
                )
            else:
                logger.warning(
                    f"Could not remove alpha '{expression}' from hopeful_alphas.json"
                )
    ...
    def run_continuous_mining(self):
        ...
        logger.info(
            f"Starting continuous alpha expression mining with {self.mining_interval/3600}h intervals"
        )
        ...
        while True:
            try:
                ...
                logger.info(
                    f"Mining cycle completed, waiting {self.mining_interval/3600} hours before next cycle..."
                )
                time.sleep(self.mining_interval)
